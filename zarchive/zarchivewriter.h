...
private:
	PathNode* GetNodeByPath(PathNode* root, const std::string_view& path);
	PathNode* FindSubnodeByName(PathNode* parent, const std::string_view& nodeName);

	uint32_t CreateNameEntry(const std::string_view& name);
