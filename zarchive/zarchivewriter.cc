...
ZArchiveWriter::PathNode* ZArchiveWriter::GetNodeByPath(ZArchiveWriter::PathNode* root, const std::string_view& path)
{
...
ZArchiveWriter::PathNode* ZArchiveWriter::FindSubnodeByName(ZArchiveWriter::PathNode* parent, const std::string_view& nodeName)
{
...
uint32_t ZArchiveWriter::CreateNameEntry(const std::string_view& name)
{
...
void ZArchiveWriter::StoreBlock(const uint8_t* uncompressedData)
{
	...
	{
		m_compressionOffsetRecord.push_back({});
		m_compressionOffsetRecord.back().baseOffset = compressedWriteOffset;
	}
